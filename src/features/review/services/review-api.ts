...
export const reviewApi = {
  ...
  getAll: (query: ReviewQuery) => {
    return axiosClient.get<ReviewPaginatedResponse>(BASE_URL, {
      params: query,
    });
  },
  ...
  // Lấy chi tiết review
  getById: (id: number) => {
    return axiosClient.get<ReviewDetail>(`${BASE_URL}/${id}`);
  },

  // Cập nhật trạng thái
  updateStatus: (id: number, data: UpdateReviewStatusData) => {
    return axiosClient.put<ReviewDetail>(`${BASE_URL}/${id}/status`, data);
  },

  // Thêm phản hồi
  createReply: (id: number, data: ReviewReplyData) => {
    return axiosClient.post<unknown>(`${BASE_URL}/${id}/reply`, data);
  },

  // Sửa phản hồi
  updateReply: (id: number, replyProductId: number, data: ReviewReplyData) => {
    return axiosClient.put<unknown>(`${BASE_URL}/${id}/reply/${replyProductId}`, data);
  },

  // Xóa phản hồi
  deleteReply: (id: number, replyProductId: number) => {
    return axiosClient.put<unknown>(`${BASE_URL}/${id}/reply/${replyProductId}`, { isDeleted: true });
  },
};
