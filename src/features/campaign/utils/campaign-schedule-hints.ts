...
function saleCommonBulletsDetailed(): string[] {
  return [
    ...
    `Earliest allowed send time: from (Promotion StartDate − ${SALE_LEAD_HOURS_BEFORE_START} hours), aligned with the promotion details view. Scheduling earlier will trigger a "too soon" error — preventing scheduling too far ahead of the sale opening (maximum 24 hours before opening). E.g., if StartDate = 06/20, 10:00 → send time cannot be before 06/19, 10:00.`,
...
function saleRegularEndDetailed(): string {
  return `For standard sales (DISCOUNT): Send time must be less than or equal to (Promotion EndDate − ${VOUCHER_END_BUFFER_HOURS} hours), aligned with the promotion view. E.g., if the promotion ends at 20:00 on June 18 → the latest allowed send time is 18:00 on June 18. Any time after that is considered too close to expiration.`;
}

function saleFlashEndDetailed(): string {
  return `For FLASH_SALE: Calculated based on the latest end time among active timeslots (maximum EndAt). Send time must be less than or equal to (that mark − ${VOUCHER_END_BUFFER_HOURS} hours). You may choose a send time that falls between two timeslots, as long as it is at least ${VOUCHER_END_BUFFER_HOURS} hours before the last timeslot ends (not necessarily before the first slot). E.g., if the last slot ends at 15:00 → latest send time is around 13:00.`;
}
...
export function getCampaignScheduleHintSections(
  ...
  const global: ScheduleHintSection = {
    ...
    bullets: [
      `Send time compared to server "now": must be at least ${SCHEDULE_MIN_LEAD_MINUTES} minutes in the future and no further than ${SCHEDULE_MAX_LEAD_DAYS} days. The quick pick "In 30 minutes" already includes this ${SCHEDULE_MIN_LEAD_MINUTES}-minute buffer.`,
    ...
    return [
      global,
      {
        ...
        bullets: [
          ...
          `Send time must be at least ${VOUCHER_END_BUFFER_HOURS} hours before the voucher expires (EndDate) to ensure delivery before expiration.`,
          `Example: If the voucher's EndDate is 17:00 on the 10th → the latest allowed send time is around 15:00 on the 10th, and must still be after the StartDate, at least ${SCHEDULE_MIN_LEAD_MINUTES} minutes in the future, and within the next ${SCHEDULE_MAX_LEAD_DAYS} days.`,
  ...
  if (rt === "SALE") {
    const bullets: string[] = [...saleCommonBulletsDetailed()];

    if (promo === "FLASH_SALE") {
      bullets.push(saleFlashEndDetailed());
    } else if (promo === "DISCOUNT" || promo.length > 0) {
      bullets.push(saleRegularEndDetailed());
    } else {
      bullets.push(
        saleRegularEndDetailed(),
        saleFlashEndDetailed(),
